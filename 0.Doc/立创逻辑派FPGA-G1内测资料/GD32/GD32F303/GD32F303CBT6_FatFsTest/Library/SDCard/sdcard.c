...
//片选信号CS输出低电平
#define SD_CS_LOW()     gpio_bit_write(GPIOA, GPIO_PIN_15,RESET)

//片选信号CS输出高电平
#define SD_CS_HIGH()    gpio_bit_write(GPIOA, GPIO_PIN_15,SET)
...
*		函数参数：无
*		返回值：	无
*		注意事项：这里有一个坑，PA15、PB3、PB4默认功能为调试端口映射，不是作为普通IO口使用。
							要使用硬件SPI2功能要先禁用默认的JTAG调试端口映射功能。首先需要使能复位时钟，然后禁用JTGA功能
*/

static void SDCard_GPIO_Config(void)
{
...
static uint8_t SD_ReadByte(void)
{
	uint8_t Data = 0;
	while (RESET == spi_i2s_flag_get(SPI2,SPI_FLAG_TBE));
	spi_i2s_data_transmit(SPI2,SD_DUMMY_BYTE);
	while(RESET == spi_i2s_flag_get(SPI2,SPI_FLAG_RBNE));
